...
let FilmService = {
    ...
    getValidAndInvalidQueryParamList : (queryParamList) => {
        ...
        queryParamList.forEach((param) => { 
            if (FILM_CONSTANTS.FILM_FILTER_PARAM.includes(param)) {
    ...
    generateDbTableColumnAndValueList: (validQueryParamList, query) => {
        ...
        validQueryParamList.forEach((param) => {
            if (param == 'title') dbTableColumnList.push(FILM_CONSTANTS.FILM_TABLE_TITLE_COLUMN); // add title column name
            // if (param == 'genre') dbTableColumnList.push(FILM_CONSTANTS.FILM_TABLE_FIRST_NAME_COLUMN); // add first name column name
            // if (param == 'actor') dbTableColumnList.push(ACTOR_CONSTANTS.ACTOR_TABLE_LAST_NAME_COLUMN); // add first name column name
            dbValuesList.push(`${query[param]}%`);
        })
        return [ dbTableColumnList, dbValuesList ];
    },
    getDbQueryTableAndValues : (req, res, next) => {
        let queryParamList = FilmService.getQueryParamList(req.query); 
        // get list of query parameters

        let [ validQueryParamList, invalidQueryParamList ] = FilmService.getValidAndInvalidQueryParamList(queryParamList);
        // get list of valid and invalid query params

        FilmService.checkIfBadRequestError(validQueryParamList, invalidQueryParamList, res);
        // if there is no valid query params, then return HTTP status code 400: BAD REQUEST

        let [ dbTableColumnList, dbValuesList ] = FilmService.generateDbTableColumnAndValueList(validQueryParamList, req.query);
